...
"""
Lifetime wealth function. Takes a dataframe and produces a series.
"""
def lifetime_wealth(df,g,rf,end_date):
    #create series for current income, income next year, etc.
    ...
    #specify assume growth rates of income.
...
"""
Function creating qctiles by lifetime wealth. Takes a dataframe, adds series
using above lifetime_wealth function and computes categorical variables.
We only create percap lifetime wealth quantiles (no use for others).
Returns original dataframe with additional series added.
"""
...
"""
def lifetime_wealth_SD(df,g,rf,end_date,num,num_age,show=0):
    ...
    for age_cat in range(num_age):
        ...
        for var2 in ['borrowers','all']:
            if var2 == 'borrowers':
                df_temp = df[(df['percap_all_loans']>0)&(df['age_cat']==age_cat)]
            else:
                df_temp = df[df['age_cat']==age_cat]
            f = lambda x: np.average(x, weights=df_temp.loc[x.index, "wgt"])
            gb = df_temp.groupby('percap_'+'LT_wealth'+'_cat{0}{1}'.format(num,age_cat))['percap_'+'all_loans'].agg(f).values
            if len(gb) < num:
                df_SD[var2].loc['LT_wealth',num+1-len(gb):] = gb
                df_SD[var2].loc['LT_wealth',:num+1-len(gb)] = gb[0]
            else:
...
"""
def lifetime_wealth_debt_count(df,g,rf,end_date,num,num_age,show=0):
    ...
    for age_cat in range(num_age):
        df_temp = df[df['age_cat']==age_cat]
        qctiles = np.array([scf_data_clean.quantile(df_temp['percap_'+'LT_wealth'],data['wgt'], j/num) for j in range(num+1)])
        qct_lists, var_names = [qctiles,debt_list],['percap_'+'LT_wealth','percap_all_loans']
        d = [pd.cut(df_temp[var_names[i]], bins=qct_lists[i],labels=range(len(qct_lists[i])-1),include_lowest=True,duplicates='drop') for i in range(2)]
        df_temp['pairs'] = list(zip(d[0], d[1]))
        SD_debt_count =df_temp.groupby(df_temp['pairs'])['wgt'].sum()
        for key in list(itertools.product(range(num), range(len(debt_brackets)))):
            if key not in list(SD_debt_count.keys()):
                print("No individuals of type",key)
                SD_debt_count = pd.concat([SD_debt_count, pd.Series([0], index=[key])])
        colors = [(0.25,0.5,1.0),(0.2,0.4,0.8),(0.15,0.3,0.6),(0.1,0.2,0.4),(0.05,0.1,0.2)]
        width = 1/6
        fig = plt.figure()
        ax = fig.add_subplot(111)
        for i in range(len(qctiles)-1):
            ...
            for j in range(len(debt_list)-1):
                if i==0:
                    ax.bar(i+1-((len(debt_list)-2)/2-j)*width,SD_debt_count[(i,j)]/norm,width,color=colors[j],label=debt_brackets[j])
                else:
                    ax.bar(i+1-((len(debt_list)-2)/2-j)*width,SD_debt_count[(i,j)]/norm,width,color=colors[j])
        ax.set_xlabel('Per-capita lifetime wealth {0}s for {1} year olds'.format(qctile_dict[num],age_labels[age_cat]))
        ax.set_title('Fraction of population')
        ax.legend()
        ax.set_ylim([0, 1])
...
def cancellation_lifetime_wealth(df,g,rf,end_date,num,num_age,show=0):
    df = lifetime_wealth_qctiles(df,g,rf,end_date,num)
    ...
    for age_cat in range(num_age):
        df_temp = df[df['age_cat']==age_cat]
        f = lambda x: np.average(x, weights=df_temp.loc[x.index, "wgt"])
        gb = df_temp.groupby('percap_'+'LT_wealth'+'_cat{0}{1}'.format(num,age_cat))
        array_temp = np.zeros((num,len(cancel_list)))
        for i, cancel in enumerate(cancel_list):
            array_temp[:,i] = gb['percap_cancel{0}'.format(cancel)].agg(f)
            width = 2/5
...
for g in g_list:
    for rf in rf_list:
        ...
        lifetime_wealth_SD(data,g,rf,end_date,num,num_age,show=1)
        lifetime_wealth_debt_count(data,g,rf,end_date,num,num_age,show=1)
        cancellation_lifetime_wealth(data,g,rf,end_date,num,num_age,show=1)
